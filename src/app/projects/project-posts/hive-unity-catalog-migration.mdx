---
title: "Legacy to Modern: Enterprise Hive-to-Unity Catalog Migration"
index: 2
...
Migrated **Hive external tables** to **Unity Catalog *managed* tables (Delta)**—consolidating storage under UC‑managed locations, standardizing governance (ABAC/RBAC), and enabling lineage & cross‑workspace sharing. Performed **DEEP CLONE/CTAS copy** with a short write freeze for cutover.
...
* **Three‑part naming everywhere**: switch from `db.table` to `catalog.schema.table` (e.g., `finance.silver.orders`).
